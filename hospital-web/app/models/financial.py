...
`payment` is a new table in the real schema — an actual payment/prepayment
ledger, separate from `invoice` (which is the bill) and `invoiceitem`
(which is the line items). trg_invoiceitem_recalculate_total and
trg_payment_update_invoice keep invoice.total_amount / insuranceAmount /
patientAmount / paidAmount / status all in sync automatically at the DB
level whenever items or payments are inserted.
"""
...
class Invoice(db.Model):
    ...
    insuranceAmount = db.Column(db.Float)   # maintained by trg_invoiceitem_recalculate_total
    patientAmount = db.Column(db.Float)     # maintained by trg_invoiceitem_recalculate_total
    paidAmount = db.Column(db.Float)        # maintained by trg_payment_update_invoice
