...
def _free_bed_choices(include_bed=None):
    # Real schema: CK_bed_status only allows 'Free' | 'Reserved' | 'Occupied'.
...
def create():
    ...
    if form.validate_on_submit():
        ...
        # Set explicitly here so the SQLite fallback backend (no T-SQL
        # triggers) behaves correctly too. On real SQL Server,
        # trg_admission_bed_occupy (their 03_triggers.sql) sets the same
        # value again — harmless — and is the one that actually REJECTS
        # the insert (RAISERROR) if the bed turned out not to be free by
        # the time the trigger runs, which the try/except below surfaces
        # as a flash message instead of a raw 500 error.
        bed.status = "Occupied"
...
def discharge(admission_id):
    ...
    if form.validate_on_submit():
        ...
        # Same dual-backend reasoning as create(): needed for the SQLite
        # fallback; redundant-but-harmless on real SQL Server where
        # trg_admission_bed_free also fires.
        if admission.bed:
...
def transfer(admission_id):
    ...
    # Same dual-backend reasoning: keeps the SQLite fallback correct;
    # redundant-but-harmless on real SQL Server where
    # trg_patienttransfer_bed_update also does this.
    if old_bed:
        old_bed.status = "Free"
