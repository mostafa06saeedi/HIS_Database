...
class Patient(db.Model):
    ...
    # Phase 2 (§5 — clinical decision support)
    allergies = db.relationship("PatientAllergy", back_populates="patient")
    # Phase 2 (§2 — patient follow-up / treatment tracking)
    followups = db.relationship("FollowUp", back_populates="patient")
...
class MedicalRecord(db.Model):
    ...
    smokingHistory = db.Column(db.String(255))
...
class DoctorDiagnosis(db.Model):
    ...
    # Phase 2 (§1 — treatment outcome is evaluated per diagnosed condition)
    outcomes = db.relationship("TreatmentOutcome", back_populates="diagnosis")
    followups = db.relationship("FollowUp", back_populates="diagnosis")
